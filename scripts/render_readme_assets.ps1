...
$stdout = Join-Path $env:TEMP ("pipeline-velocity-dash-" + [guid]::NewGuid().ToString() + "-stdout.log")
$stderr = Join-Path $env:TEMP ("pipeline-velocity-dash-" + [guid]::NewGuid().ToString() + "-stderr.log")
$port = "5298"
$process = $null
$edgeCandidates = @(
...
try {
    ...
    $targets = @(
        ...
        @{ Url = "http://127.0.0.1:$port/stage-funnel"; File = "02-stage-funnel-proof.png"; Size = "1600,1300" },
        @{ Url = "http://127.0.0.1:$port/segment-velocity"; File = "03-segment-velocity-proof.png"; Size = "1600,1360" },
